...
1.Add animated intro
2.Set owner drawn controls
3.Add matrix as a new menu
4.Fix bugs
*/
/*Just a random thought...
what if we could use complex numbers in matrix calculation?
Maybe we should merge the complex mode with the calculate mode.
The work of a new version should be started!
*/
#include<windows.h>
#include"resource.h"
#include<bits/stdc++.h>
...
void createstatics(HWND f)
{
    ...
    CreateWindowEx(WS_EX_CLIENTEDGE,
                   "Button","Matrix",
                   WS_VISIBLE|WS_CHILD,
                   160,0,80,30,f,(HMENU)ID_MAT,GetModuleHandle(NULL),NULL);
    CreateWindowEx(WS_EX_CLIENTEDGE,
                      "Edit","",
    ...
    case WM_SIZE:
        {
            ...
            SetWindowPos(client,NULL,(sr/5)*2,0,sr/5,sb*8/100,SWP_NOZORDER);
    ...
    case WM_COMMAND:
        {
            ...
            case ID_COMP:
                ...
                f=8;
                if(ret==ID_POL)
                    SetWindowText(GetDlgItem(h,ID_MODE),"Comp-Pol");
                else
                    SetWindowText(GetDlgItem(h,ID_MODE),"Comp-Rec");
                SendMessage(h,WM_SIZE,0,0);
                }
                      break;
            case ID_MAT:
                ...
                if(string(dj)=="Calculate"||string(dj)=="Mat")
